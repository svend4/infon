...
| `rayscene` | `{prompt}` | `ray` (a scene graph: `{objects[]{kind:sphere\|box\|pyramid\|cylinder\|tree\|house\|mesh\|plane,name,x,y,z,r,s:[hx,hy,hz],color,emit,glass,metal,reflect,rough}, light, skyTop, skyBottom}`; `kind:mesh` instances a named model — `name` ∈ the renderer's mesh library, e.g. `crystal`, `rock`, plus any `.obj` loaded) |
